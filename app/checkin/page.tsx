...
export default function CheckinPage() {
  ...
  const handleRealNfcTap = async () => {
    // Clear logs and start logging immediately
    clearNfcLogs();
    addNfcLog('🚀 User clicked "Tap Phone to Check In"');
    addNfcLog('🧹 Clearing previous state...');

    setNfcScanning(true);
    setErrorMessage(null);
    setRealError(null);

    addNfcLog('🔧 Initializing NFC scanner...');

    try {
      addNfcLog('📞 Calling nfcService.startScanning()...');
      await nfcService.startScanning(
        // Success callback
        async (token: string) => {
          addNfcLog('✅ SUCCESS CALLBACK TRIGGERED!');
          addNfcLog('🎫 Token received:', token);
          console.log('NFC token read:', token);
          setNfcScanning(false);
          nfcService.stopScanning();

          // Process the NFC check-in
          addNfcLog('📡 Starting API check-in process...');
          setNfcLoading(true);
          try {
            const nexusNumber = await getNexusNumberFromStorage();
            addNfcLog('🏥 Clinic Nexus Number:', nexusNumber);

            addNfcLog('📤 Sending POST request to /mobile/checkin/nfc');
            ...
            addNfcLog('📥 API Response received', response.data);

            if (response.data && Array.isArray(response.data) && response.data.length > 0) {
              addNfcLog('✅ Bookings found!', { count: response.data.length });
              ...
              addNfcLog('🎉 Success! Redirecting to appointments...');
              ...
              addNfcLog('⚠️ Server message:', response.data.message);
              setErrorMessage(response.data.message as string);
            } else {
              addNfcLog('❌ No appointment found in response');
              setErrorMessage('NFC check-in failed. No appointment found.');
            }
          } catch (error: unknown) {
            addNfcLog('❌ API ERROR occurred!', error);
            ...
            if (error && typeof error === 'object' && 'response' in error) {
              ...
              if (axiosError.response?.data) {
                const data = axiosError.response.data;
                addNfcLog('🔍 Error response data:', data);
        ...
        // Error callback
        (error: string) => {
          addNfcLog('❌ ERROR CALLBACK TRIGGERED!');
          addNfcLog('💥 Error message:', error);
          console.error('NFC scanning error:', error);
          setNfcScanning(false);
          setErrorMessage(error);
          nfcService.stopScanning();
        },
        // Log callback - this receives logs from nfcService
        (message: string, data?: unknown) => {
          addNfcLog(message, data);
        }
      );
    } catch (error) {
      addNfcLog('❌ EXCEPTION CAUGHT in handleRealNfcTap!');
      addNfcLog('💥 Exception details:', error);
    ...
    <div className="min-h-screen bg-white text-gray-900 flex flex-col p-8">
      ...
      <main className="flex flex-col items-center text-center flex-grow w-full">
        ...
        <div className='min-w-3/5 max-w-[50rem]'>
          ...
          {/* NFC Debug Logs - Always visible when NFC is supported */}
          {nfcSupported && nfcLogs.length > 0 && (
            <div className="rounded-2xl p-6 mb-8 bg-gray-900 text-green-400 font-mono text-sm max-h-96 overflow-y-auto shadow-xl">
              <div className="flex justify-between items-center mb-4 sticky top-0 bg-gray-900 pb-2 border-b border-gray-700">
                <h3 className="text-lg font-semibold text-white">🔍 NFC Debug Logs</h3>
                <button
                  onClick={clearNfcLogs}
              ...
              <div className="space-y-2">
                {nfcLogs.map((log, index) => (
                  <div key={index} className="border-l-2 border-green-500 pl-3 py-1">
                    ...
                    {log.data !== undefined && (
                      <pre className="text-yellow-300 text-xs mt-1 overflow-x-auto bg-gray-800 p-2 rounded">
